window.addEventListener('load', () => {
  ...
  function resizeCanvases() { /* die beiden canvases je nach seitenverhältnis anordnen */
    ...
    if (portrait) {
      ...
      canvas2.style.top = canvas1.height + 'px'; // unter canvas1 positionieren wenn hochkant
      canvas2.style.left = '0'; // linksbündig
    } else {
      canvas1.width = window.innerWidth / 2;
      canvas1.height = window.innerHeight;
      canvas2.width = window.innerWidth / 2;
      canvas2.height = window.innerHeight;
      canvas2.style.left = canvas1.width + 'px'; // rechts von canvas1 positionieren wenn quer
      canvas2.style.top = '0'; // oben bündig
    }
  ...
  function drawBranch(ctx, cfg, level) { /* Zentrale Funktion, um einen Ast zu zeichnen. Ist rekursiv mit Abbruchbedingung nach levels */
    ...
    for (let i = 0; i < cfg.branching; i++) { /* Für jede Verzweigung: speichern des aktuellen Zustands, verschieben und skalieren des Koordinatensystems */
      ...
      if (cfg.nLevelBranches === 2) { // falls 2 Äste pro Ebene, zweiten Ast zeichnen
        ctx.save();
        ctx.rotate(-cfg.spread);
        drawBranch(ctx, cfg, level + 1);
        ctx.restore();
  ...
  function drawTarget() {
    if (!target) return; 
    const size = Math.min(canvas2.width, canvas2.height) * 0.22; /* Größe des Fraktals abhängig von der Canvasgröße, hier auf 22% der kleineren canvas-dimension gesetzt */
    ...
    Object.keys(target).forEach(k => { // je nach Art des Werts unterschiedlich runden
      if (['sides','levels','branching','nLevelBranches','lineWidth','hue'].includes(k)) rounded[k] = Math.round(target[k]); // ganze Zahlen
      else if (['scale','spread','lineLength','rotation'].includes(k)) rounded[k] = Math.round(target[k]*10)/10; // eine Nachkommastelle
      else rounded[k] = target[k]; // alle anderen Werte unverändert übernehmen
    });

    drawFractal(ctx2, { ...rounded, size, color: `hsl(${rounded.hue},100%,50%)` }); // ...rounded = Array wird in einzelne key-value Paare aufgeteilt und in das Objekt eingesetzt
  }
  ...
  function generateTargetByDifficulty(level) {
    const cfg = [ /* Konfigurationen für die verschiedenen Schwierigkeitsgrade */
      ...
      {sides:[5,8],levels:[1,2],branching:[1,2],n:0.2}, 
      {sides:[7,12],levels:[1,3],branching:[1,2],n:0.5}, 
      {sides:[8,12],levels:[2,4],branching:[2,3],n:0.8},
      {sides:[10,15],levels:[3,4],branching:[2,3],n:0.9}
    ][level];

    target = { // Ziel-Fraktal mit zufälligen Werten innerhalb der cfg-Grenzen generieren
      sides: rand(...cfg.sides), // das "..." ist ein Spread-Operator, der das Array in einzelne Argumente aufteilt
      levels: rand(...cfg.levels),
      branching: rand(...cfg.branching),
      nLevelBranches: chance(cfg.n) ? 2 : 1, // nutzt die chance-Funktion mit der Wahrscheinlichkeit cfg.n
      scale: Math.round(rand(0.35,0.6,0.05)*10)/10,
      spread: Math.round(rand(-3.2,3.2,0.1)*10)/10,
      lineWidth: rand(5,30,5),
    ...
    if(level === 4) {
      target.lineLength = Math.round(rand(0.4,1.2,0.1)*10)/10;
  ...
  function checkMatch() { /* Überprüfen, ob der Spieler das Ziel-Fraktal getroffen hat, wird nach jeder Änderung des Spielerzustands aufgerufen */
    ...
    const keys = ['sides', 'levels', 'lineWidth', 'lineLength', 'hue'];
    if (state.levels > 0) { // nur wenn levels > 0 sind diese Parameter relevant, da sonst keine Äste gezeichnet werden
      keys.push('spread', 'scale', 'branching', 'nLevelBranches');
    }
    if(keys.every(k=>state[k]===target[k])){
      successMessage.style.display='block'; // Erfolgsmeldung anzeigen
      screenFlash.classList.add('active'); // Flash-Klasse hinzufügen für Animation
      setTimeout(()=>{screenFlash.classList.remove('active');},400); // Flash-Klasse nach 100ms wieder entfernen, damit Animation zurückgesetzt wird
      setTimeout(()=>{successMessage.style.display='none';},2000); // Erfolgsmeldung nach 2s wieder ausblenden
      gameMode=false; // Spielmodus beenden, damit nicht mehrfach Erfolge erkannt werden
      randomizeButton.disabled=false; // "Neues Ziel"-Button wieder aktivieren
    }
  }
